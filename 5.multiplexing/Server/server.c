...
int main()
{
    ...
    while(1){
        ...
        /*3. check whether there are new clients trying to connect the server. If so, we accept the connections. */
        ...
        /*4. check other existing sockets, and process their events*/
        for(int i=0; i<SOCKETLIST_LENGTH; i++){
            ...
            if(FD_ISSET(newsock, &readfds)){
                ...
                // edit the received message
                strcpy(sendbuf, recvbuf);
                strcat(sendbuf, "(echoed by the server)");

                if (send(sock_list.sock_array[i], sendbuf, strlen(sendbuf)+1, 0) == -1) {
                    perror("send");
                }
            }
        }
